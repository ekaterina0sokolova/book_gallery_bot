...
|-----------|----------|----------|-----------|----------------|---------------------|
|1.1 | test_create_book | Проверяет, что функция инициализации создаёт объект книги с корректными параметрами | Позитивный | title="title1" author="auth1" tag="tag1" | Объект книги успешно создан, без исключений |
...
|1.4 | test_create_book_with_blank_tag | Проверяет, что функция инициализации выбрасывает исключение при передаче пустого тега. | Негативный | title="title1" author="auth1" tag="" | Исключение `ValueError` с сообщением "Тэг книги не может быть пустым." |
...
|2.12 | test_get_book_by_id | Проверяет, что функция `get_book_by_id` возвращает книгу по указанному `book_id`. | Позитивный | `book_id` существующей книги | Возвращаемое значение - объект `Book` с заданным `book_id` ||2.12 | test_get_book_by_id | Проверяет, что функция `get_book_by_id` возвращает книгу по указанному `book_id`. | Позитивный | `book_id` существующей книги | Возвращаемое значение - объект `Book` с заданным `book_id` |
...
|Номер теста| Название | Описание | Тип теста | Входные данные | Ожидаемый результат |
|-----------|----------|----------|-----------|----------------|---------------------|
|3.1 | test_start_command | Проверяет корректность отправки стартового сообщения при команде /start | Позитивный | Мок объекты update и context | Отправка приветственного сообщения с описанием доступных команд |
...
|3.4 | test_book_action_book_invalid | Проверяет корректный вывод пользоватльского сообщения, если книга не найдена | Негативный | callback_query.data = "book_nonexistent" | Книга успешно добавлена в каталог, возвращает ConversationHandler.END, Выводится текстовое сообщение "Книга не найдена." |
|3.5 | test_book_action_unread_book_success | Проверяет корректный вывод пользовательского сообщения, если вызван book_action_handler для отметки кнпиги, как "Прочитанную" | Позитивный | callback_query.data = "book_123", is_read.return_value = False | Книга отмечена, как прочитанная, пользователю выведено сообщение "Книга отмечена как прочитанная." |
|3.6 | test_book_action_unread_book_invalid | Проверяет корректный вывод сообщения пользователю, если книгу не удалось отметить как "Прочитанную" | Негативный | callback_query.data = "book_123", is_read.return_value = True | Книга не отмечена как прочитанная, пользоватлею выведено сообщение "Не удалось отметить книгу как прочитанную." |
|3.7 | test_book_action_remove_book | Проверяет корректный вывод сообщения пользователю при успешном удалении книги. | Позитивный | callback_query.data = "book_123", remove_book.return_value = True | Книга удалена из списка книг пользоватлея, ему выведено сообщение "Книга удалена из каталога." |
|3.8 | test_book_action_remove_book_invalid | Проверяет корректный вывод сообщения пользователю при ошибке удаления книги. | Негативный | callback_query.data = "book_123", remove_book.return_value = False | Книга не удалена из списка книг пользоватлея, ему выведено сообщение "Не удалось удалить книгу из каталога." |